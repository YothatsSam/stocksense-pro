    ...
    const result = await Promise.all(recipes.map(async (recipe) => {
      const { rows: ingredients } = await pool.query(`
        ...
        LEFT JOIN stock_levels sl ON sl.product_id = ri.product_id
                                 ...
                                 AND sl.organisation_id = $3
        WHERE ri.recipe_id = $1
      `, [recipe.id, recipe.location_id, orgId])
...
async function createRecipe(req, res, next) {
  ...
  try {
    ...
    if (locCheck.length === 0) {
      ...
      return res.status(403).json({ error: 'Location not found.' })
    }
...
async function serveRecipe(req, res, next) {
  ...
  try {
    ...
    const { rows: recipe } = await client.query(
      'SELECT * FROM recipes WHERE id = $1 AND organisation_id = $2',
      [recipeId, orgId]
    )
    if (recipe.length === 0) {
      await client.query('ROLLBACK')
    ...
    const { rows: ingredients } = await client.query(`
      ...
      FROM recipe_ingredients ri
      JOIN stock_levels sl ON sl.product_id = ri.product_id
                          AND sl.location_id = $2
                          AND sl.organisation_id = $3
      JOIN products p ON p.id = ri.product_id
      WHERE ri.recipe_id = $1
    `, [recipeId, locationId, orgId])
    ...
    for (const ing of ingredients) {
      await client.query(
        ...
        [ing.quantity_required, ing.product_id, locationId, orgId]
      )
      await client.query(
        `INSERT INTO stock_movements (organisation_id, product_id, location_id, quantity_change, reason)
         VALUES ($1, $2, $3, $4, 'served')`,
        [orgId, ing.product_id, locationId, -Number(ing.quantity_required)]
      )
    }
...
async function getProducts(req, res, next) {
  ...
  try {
    const { rows } = await pool.query(
      'SELECT id, sku, name, unit, unit_cost FROM products WHERE organisation_id = $1 ORDER BY name',
      [orgId]
    )
    res.json(rows)
  } catch (err) {
